<#
.SYNOPSIS
    PostToolUse hook: regenerate the matching .htm when a Claudia_v*.md is edited.

.DESCRIPTION
    Reads the hook's JSON payload from stdin, checks whether the touched path is
    a Claudia_v*.md in this repo, and if so re-runs build-html.ps1 on it.
...
# Only react to Claudia_v*.md edits inside this repo.
$repoRoot = Split-Path -Parent $PSScriptRoot
$leaf = Split-Path -Leaf $path
if ($leaf -notmatch '^Claudia_v\d+\.md$') { exit 0 }
